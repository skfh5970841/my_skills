...
def build_generation_prompt(case: dict, skill_path: Path) -> str:
    ...
    return (
        ...
        f"Selected local SKILL.md: {skill_path}\n\n"
...
def generate_cases(
    ...
    prepared: list[tuple[dict[str, str], Path, dict[str, str]]] = []
    ...
    for raw in raw_cases:
        ...
        skill, entrypoint = _skill_tree(root, case["target_skill"])
        prepared.append((case, entrypoint, _skill_snapshot(skill)))

    rows: list[dict] = []
    for case, entrypoint, source_snapshot in prepared:
        prompt = build_generation_prompt(case, entrypoint)
        for repeat in range(repeats):
            started_at = datetime.now(timezone.utc).isoformat()
            result = run_command(replace(config, stdin_text=prompt))
            rows.append(
                {
